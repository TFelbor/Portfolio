...
def longestCommonPrefix(strs: list[str]) -> str:
    ...
    while (not mismatch):
        ...
        for index, value in enumerate(strs):
            ...
            #If it's a match and it's the last string of the array, add the common letter to the prefix and repeat the
            #process with the next letter in the index
            if(letterToCheck == value[letterIndex] and index == (len(strs) - 1)):
                ...
                letterIndex += 1
           #If it's a match, but it's not the last string of the array, continue to check the same character in
